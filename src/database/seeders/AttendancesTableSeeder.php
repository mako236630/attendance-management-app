...
class AttendancesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($users as $user) {
            ...
            $beforeYesterday = Carbon::now()->subDays(2);
            if (!$beforeYesterday->isWeekend()) {
                $atd1 = Attendance::create([
                    'user_id' => $user->id,
                    'status'  => 1,
                    'punched_in_at'   => $beforeYesterday->format('Y-m-d') . ' 09:00:00',
                    'punched_out_at'  => $beforeYesterday->format('Y-m-d') . ' 18:00:00',
                    'requested_in_at' => $beforeYesterday->format('Y-m-d') . ' 10:00:00',
                    'requested_out_at' => $beforeYesterday->format('Y-m-d') . ' 18:00:00',
                    'note' => '電車遅延の為',
                ...
                Rest::create([
                    ...
                    'rest_in_at' => $beforeYesterday->format('Y-m-d') . ' 12:00:00',
                    'rest_out_at' => $beforeYesterday->format('Y-m-d') . ' 13:00:00',
                    'requested_in_at' => $beforeYesterday->format('Y-m-d') . ' 12:00:00',
                    'requested_out_at' => $beforeYesterday->format('Y-m-d') . ' 13:00:00',
                ]);

                Rest::create([
                    'attendance_id' => $atd1->id,
                    'rest_in_at' => $beforeYesterday->format('Y-m-d') . ' 15:00:00',
                    'rest_out_at' => $beforeYesterday->format('Y-m-d') . ' 16:00:00',
                    'requested_in_at' => $beforeYesterday->format('Y-m-d') . ' 17:00:00',
                    'requested_out_at' => $beforeYesterday->format('Y-m-d') . ' 17:30:00',
                ]);
            }
